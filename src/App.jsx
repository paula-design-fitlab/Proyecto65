...
const RECETAS = [
  ...
  { id:11, nombre:'Gazpacho + pavo a la plancha', tiempo:5, proteina:'Pavo',         taper:true,  batch:false, ingredientes:['Gazpacho (tetrabrik)','Pavo en filetes','AOVE'], calorias:290 },
  { id:12, nombre:'Crema fría de calabacín', tiempo:20, proteina:'Huevos',           taper:true,  batch:true,  ingredientes:['Calabacín','Cebolla','Quesito','Caldo de verduras'], calorias:180 },
  { id:13, nombre:'Yogur griego + fruta', tiempo:2, proteina:'Huevos',               taper:true,  batch:false, ingredientes:['Yogur griego natural','Fruta de temporada'], calorias:160 },
  { id:14, nombre:'Huevos revueltos con champiñones', tiempo:8, proteina:'Huevos',   taper:false, batch:false, ingredientes:['Huevos','Champiñones','AOVE','Sal'], calorias:310 },
]
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-titulo span { font-style: italic; color: #E8956D; }
  ...
  .nav-dot { width: 4px; height: 4px; border-radius: 50%; background: #E8956D; margin-top: 2px; opacity: 0; }
  ...
  .comida-acciones { display: flex; gap: 8px; margin-top: 12px; padding-top: 10px; border-top: 1px solid ${P.borde}; }
  ...
  .nota-input:focus { border-color: #B5C9B7; }
  .estrellas { display: flex; gap: 4px; margin-top: 8px; }
  ...
  .batch-paso { background: ${P.fondoCard}; border: 1px solid ${P.borde}; border-radius: 16px; padding: 14px 16px; margin-bottom: 10px; display: flex; gap: 12px; align-items: flex-start; }
  .batch-num { width: 28px; height: 28px; border-radius: 50%; background: ${P.melocotonSuave}; color: #B5643A; font-size: 13px; font-weight: 600; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .batch-check { width: 28px; height: 28px; border-radius: 50%; border: 2px solid ${P.borde}; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: all 0.2s; font-size: 14px; }
  .batch-check.hecho { background: ${P.sage}; border-color: ${P.sage}; color: white; }
`
  ...
  return (
    <div>
      {texto.split('\n').map((l, i) => {
        ...
        if (l.match(/^[-–•] /))  return <div key={i} style={{ display:'flex', gap:8, marginBottom:3 }}><span style={{ color:'#E8956D', flexShrink:0 }}>·</span><span style={{ fontSize:14, lineHeight:1.6 }} dangerouslySetInnerHTML={{ __html: l.replace(/^[-–•] /,'').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>') }} /></div>
...
function Hoy({ menuSemanal, pesoRegistros, registroHoy, setRegistroHoy }) {
  ...
  const toggleComido = async (tipo) => {
    const reg = getReg(tipo)
    const comido = !(reg?.comido)
    const { data } = await supabase.from('p65_registro_diario')
      .upsert({ usuario:'paula', fecha:fechaStr, tipo, comido, nota: reg?.nota || null, puntuacion: reg?.puntuacion || null }, { onConflict:'usuario,fecha,tipo' })
      .select()
    if (data) setRegistroHoy(prev => { const f = prev.filter(r => r.tipo !== tipo); return [...f, data[0]] })
  }

  const setPuntuacion = async (tipo, puntuacion) => {
    const reg = getReg(tipo)
    const { data } = await supabase.from('p65_registro_diario')
      .upsert({ usuario:'paula', fecha:fechaStr, tipo, comido: reg?.comido || false, nota: reg?.nota || null, puntuacion }, { onConflict:'usuario,fecha,tipo' })
      .select()
    if (data) setRegistroHoy(prev => { const f = prev.filter(r => r.tipo !== tipo); return [...f, data[0]] })
  }

  const guardarNota = async (tipo) => {
    const reg = getReg(tipo)
    const { data } = await supabase.from('p65_registro_diario')
      .upsert({ usuario:'paula', fecha:fechaStr, tipo, comido: reg?.comido || false, nota: notaTexto, puntuacion: reg?.puntuacion || null }, { onConflict:'usuario,fecha,tipo' })
      .select()
    if (data) setRegistroHoy(prev => { const f = prev.filter(r => r.tipo !== tipo); return [...f, data[0]] })
    setNotaAbierta(null)
  }
  ...
  return (
    <>
      ...
      <div className="content">
        {comidas_hechas > 0 && (
          <div style={{ background: P.sageSuave, borderRadius:14, padding:'10px 14px', marginBottom:4, display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div key={tipo} className={`comida-card ${hecha ? 'hecha' : proxima === tipo && !hecha ? 'proxima' : ''}`}>
              <div className="comida-top">
                ...
                <div style={{ flex:1 }}>
                  ...
                  {reg?.puntuacion && (
                    <div style={{ fontSize:12, marginTop:2 }}>{'⭐'.repeat(reg.puntuacion)}</div>
                  )}
                </div>
              </div>
              <div className="comida-acciones">
                <button className={`check-btn ${hecha ? 'activo' : ''}`} onClick={() => toggleComido(tipo)}>
                  {hecha ? '✓ Comido' : 'Marcar'}
                </button>
                <div style={{ display:'flex', gap:4 }}>
                  {[1,2,3,4,5].map(n => (
                    <span key={n} className={`estrella ${(reg?.puntuacion||0) >= n ? 'on' : ''}`} onClick={() => setPuntuacion(tipo, n)}>⭐</span>
                  ))}
                </div>
                <button className="check-btn check-btn-nota" onClick={() => abrirNota(tipo)}>
                  {reg?.nota ? '📝 Editar nota' : '+ Nota'}
                </button>
              </div>
              {notaAbierta === tipo && (
                <div style={{ marginTop:10 }}>
                  ...
                  <div style={{ display:'flex', gap:8, marginTop:6 }}>
                    <button className="btn btn-secundario" style={{ padding:'8px 14px', fontSize:13 }} onClick={() => setNotaAbierta(null)}>Cancelar</button>
                    <button className="btn btn-primario" style={{ padding:'8px 14px', fontSize:13 }} onClick={() => guardarNota(tipo)}>Guardar</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="card card-crema">
          ...
          <div className="progreso-nums"><span>{PERFIL.pesoInicial} kg</span><span style={{ color:'#E8956D', fontWeight:500 }}>{progreso.toFixed(0)}%</span><span>{PERFIL.pesoObjetivo} kg</span></div>
        </div>
...
function Menu({ menuSemanal, setMenuSemanal, guardarMenu }) {
  ...
  const abrirCambio = (tipo) => { setEditando(tipo); setRecetaSeleccionada(null); setFiltroRapido('') }

  const confirmarCambio = async () => {
    if (!recetaSeleccionada) return
    const nuevo = { ...menuSemanal, [diaActivo]: { ...(menuSemanal[diaActivo] || MENU_BASE[diaActivo]), [editando]: recetaSeleccionada.nombre } }
  ...
  const recetasFiltradas = RECETAS.filter(r => {
    ...
    return r.nombre.toLowerCase().includes(filtroRapido.toLowerCase()) || r.proteina.toLowerCase().includes(filtroRapido.toLowerCase())
  })
  ...
  return (
    <>
      ...
      <div className="content">
        ...
        {comidas.map(({ tipo, emoji, hora, cls, label }) => (
          <div key={tipo} className="comida-card" style={{ cursor:'pointer' }} onClick={() => abrirCambio(tipo)}>
            <div className="comida-top">
              ...
              <div style={{ fontSize:13, color:P.textoSuave, flexShrink:0 }}>Cambiar →</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {editando && (
        <div className="modal-overlay" onClick={() => setEditando(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <input className="input" placeholder="Buscar receta o proteína..." value={filtroRapido} onChange={e => setFiltroRapido(e.target.value)} style={{ marginBottom:12 }} />
            <div style={{ maxHeight:340, overflowY:'auto' }}>
              {recetasFiltradas.map(r => (
                <div key={r.id} className={`receta-card ${recetaSeleccionada?.id === r.id ? 'sel' : ''}`} onClick={() => setRecetaSeleccionada(r)}>
            ...
            <div style={{ display:'flex', gap:10, marginTop:12 }}>
              ...
              <button className="btn btn-primario" onClick={confirmarCambio} disabled={!recetaSeleccionada}>Confirmar cambio</button>
            </div>
          </div>
        </div>
      )}
  ...
  if (detalle) return (
    <div className="receta-detalle">
      <div className="receta-detalle-header">
        ...
        <div><div style={{ fontSize:11, color:P.textoSuave, marginBottom:2 }}>Receta</div><div style={{ fontSize:18, fontWeight:600 }}>{detalle.nombre}</div></div>
      </div>
  ...
  return (
    <>
      ...
      <div className="content">
        ...
        <div className="chip-wrap">{proteinas.map(p => <button key={p} className={`chip ${filtroP===p?'sel':''}`} onClick={() => setFiltroP(filtroP===p?'':p)}>{p}</button>)}</div>
        <p className="seccion">Tiempo</p>
        <div className="chip-wrap">{['≤10 min','≤20 min','≤30 min','Cualquiera'].map(t => <button key={t} className={`chip ${filtroT===t?'sel':''}`} onClick={() => setFiltroT(filtroT===t?'':t)}>{t}</button>)}</div>
        <p className="seccion">Condiciones</p>
        <div className="chip-wrap">{['Para táper','Batch cooking'].map(c => <button key={c} className={`chip ${filtroCond===c?'sel':''}`} onClick={() => setFiltroCond(filtroCond===c?'':c)}>{c}</button>)}</div>
...
function QueComoHoy({ menuSemanal }) {
  ...
  const buscar = async () => {
    ...
    const prompt = `Eres el asistente nutricional de Proyecto 65 para Paula (24a, 160cm, 74.5kg→65kg, fuerza 3-4días/semana).
Horario: comida 12:30, merienda 17:00 táper, cena 00:00. Le gustan: pollo, solomillo, salmón, merluza, atún, bonito, gambas, sepia, huevos. No: pepino, espinacas.
Objetivo: ~1550kcal/día, ~130g proteína. Filosofía: nunca hambre, comida normal, mínima preparación.
...
Sugiere 2-3 opciones concretas con: nombre, ingredientes principales, tiempo, si sirve para táper. Sé directo y práctico. En español.`
    try {
      ...
      <div className="content">
        {modo === 'resultado' ? (
          ...
          <>
            ...
            <p className="seccion">Condiciones</p>
            <div className="chip-wrap">{['Para táper','Batch cooking','Sin cocinar','Plato único'].map(v => <button key={v} className={`chip ${filtros.cond===v?'sel':''}`} onClick={() => toggle('cond',v)}>{v}</button>)}</div>
...
function BatchCooking() {
  ...
  const generarPlan = async () => {
    ...
    setCargando(true); setModo('plan'); setPlan(''); setPasosHechos([])
    const lista = seleccionadas.map(r => `- ${r.nombre} (${r.tiempo} min, ingredientes: ${r.ingredientes.join(', ')})`).join('\n')
    const prompt = `Eres el asistente de batch cooking de Proyecto 65 para Paula.
Va a preparar estas recetas el domingo:
${lista}

Crea un plan de cocina optimizado para el domingo. Organiza las tareas en orden inteligente (primero lo que va al horno, mientras tanto preparar lo demás, etc). Para cada paso indica:
- Número de paso
- Qué hacer exactamente  
- Tiempo estimado
- Consejo práctico si aplica

Al final indica el tiempo total aproximado y cómo conservar cada preparación (nevera/congelador y cuántos días). Sé práctico y concreto. En español.`
    try {
      const res = await fetch('/.netlify/functions/claude', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ messages:[{ role:'user', content:prompt }] }) })
      const data = await res.json()
      setPlan(data.content?.[0]?.text || 'No pude generar el plan.')
    } catch { setPlan('Error al conectar. Inténtalo de nuevo.') }
    setCargando(false)
  }
        ...
        {modo === 'plan' ? (
          ...
          <>
            <div className="card card-lavanda" style={{ marginBottom:20, padding:'18px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:13, color:P.textoMedio }}>Elige las recetas y la IA te organiza el orden perfecto</div>
            </div>
            <p className="seccion">Recetas para batch cooking</p>
            {recetasBatch.map(r => (
...
function Perfil({ pesoRegistros, guardarPeso }) {
  ...
  const handle = async () => {
    ...
    setGuardando(true); await guardarPeso(parseFloat(nuevoPeso)); setNuevoPeso(''); setGuardando(false)
  }
  return (
    <>
      ...
      <div className="content">
          ...
          <>
            ...
            <div className="card">
              {[...pesoRegistros].reverse().slice(0,8).map((r,i,arr) => {
                const prev = arr[i+1]; const diff = prev ? +(r.peso-prev.peso).toFixed(1) : null
                return (
                  <div key={r.id} className="peso-item">
                    <div><div className="peso-val">{r.peso} kg</div><div className="peso-fecha">{new Date(r.fecha).toLocaleDateString('es-ES',{weekday:'short',day:'numeric',month:'short'})}</div></div>
                    {diff!==null && <div className={`peso-diff ${diff<0?'tag-verde':diff>0?'tag-rojo':''}`}>{diff<0?'↓':diff>0?'↑':'–'} {Math.abs(diff)} kg</div>}
                  </div>
                )
              })}
            </div>
        ...
        <div className="card">
          {['Nunca pasar hambre','No prohibir alimentos','Mínima cocina entre semana','Batch cooking dominical','Sostenible durante años'].map(f => (
            <div key={f} className="pill-filosofia"><div className="filosofia-check">✓</div><span style={{ fontSize:14 }}>{f}</span></div>
          ))}
        </div>
      </div>
    </>
